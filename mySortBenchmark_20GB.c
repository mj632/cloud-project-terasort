...
#define FILE_SIZE 200000000		//--------2GB size
#define LINES 	  20000000		//-------FILE_SIZE/NO_OF_FILES
...
#define OUT_BUFF 2						//-------- Output buffer size multiplier
...
*/
	...
	for(int fi = 0; fi < NO_OF_FILES; fi++)
	{
		...
		fclose(inputFileptr);
		count[fi]++;				//-------DO like this
	}

//-----------------Done as per my understanding-------------//
	outputFileptr = fopen("/tmp/mpateloutput", "w");
	fclose(outputFileptr);
	long int opos = 0, output_buff_count = 0;
	int min_i = 0;
	
	while(output_buff_count < ((long int)FILE_SIZE/(OUT_BUFF*buffSize)))				//------ avoid operations 20000/500
	{
...
*/			
	...
	//-----------------sort here and save it in either file or array-----------//
	gettimeofday(&process_end_time, NULL);
	total_time_taken = (float) (process_end_time.tv_usec - process_start_time.tv_usec) / 1000000 + (float) (process_end_time.tv_sec - process_start_time.tv_sec);
...
void *assign_work(void *arg)
{
	...
	inputBuff = (char **) calloc((long int)LINES, sizeof(char *));			//---------Try not to get float value

	while(block_no < NO_OF_FILES)
	{
		inputFileptr = fopen("/input/data-20GB.in", "r");
		pos = (long long int)block_no * TOTAL_CHAR * LINES;				//-----LINES = 2500 or LINES/2
